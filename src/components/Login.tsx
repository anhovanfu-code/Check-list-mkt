    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        <div className="flex justify-center animate-bounce">
          <div className="h-16 w-16 rounded-2xl bg-blue-600 flex items-center justify-center shadow-lg shadow-blue-500/25 border border-blue-400">
            <ShieldCheck className="h-9 w-9 text-white" />
          </div>
        </div>
        <h2 className="mt-6 text-center text-2xl font-extrabold text-slate-900 tracking-tight">
          Hệ Thống Phân Quyền & Bảo Mật Lương
        </h2>
        <p className="mt-2 text-center text-sm text-slate-500 max-w">
          Nhóm Media & Marketing — Công ty Fugalo & Anh Long
        </p>
      </div>

      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md animate-in fade-in slide-in-from-bottom-5 duration-300">
        <div className="bg-white py-8 px-4 shadow-sm rounded-2xl border border-slate-200 sm:px-10 space-y-6">
          ...
          <div className="space-y-4">
            <div className="text-center space-y-2">
              <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-blue-50 text-blue-700 border border-blue-200 uppercase tracking-wider">
                Xác Thực Chính Thức (Firebase Auth)
              </span>
              <p className="text-xs text-slate-500 leading-relaxed">
                Để bảo vệ an toàn dữ liệu lương, thưởng và KPIs, bạn cần đăng nhập thông qua cổng xác thực bảo mật Google Accounts.
              </p>
            </div>
            ...
            <button
              ...
              className="w-full flex justify-center items-center gap-3 py-3.5 px-4 bg-white hover:bg-slate-50 border border-slate-300 hover:border-slate-400 rounded-xl shadow-xs text-sm font-bold text-slate-700 transition duration-150 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isAuthenticating ? (
                <>
